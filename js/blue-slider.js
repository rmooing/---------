(function ($) {
  $.fn.blue_slider = function (options) {
    ...
    var sliderParentWidth =
      parseInt(sliderParent.css("width")) -
      (settings.right_padding + settings.left_padding);
    ...
    for (var s = 0; s < slide_template.length; s++) {
      ...
      if (indexBegin > -1) {
        ...
        var repeatCount = parseInt(
          slide_template.substring(indexBegin, indexComma),
        );
        var repeatValue =
          " " + slide_template.substring(indexComma, indexEnd).trim() + " ";
        var tempSlide_template = slide_template.substring(0, indexBegin - 1);
        for (var r = 0; r < repeatCount; r++) {
          tempSlide_template += repeatValue;
        }
        slide_template =
          tempSlide_template + slide_template.substring(indexEnd + 1);
        s = indexBegin;
      }
    }
    ...
    if (current_fr_index > activeItemsCount || current_fr_index < 1) {
      console.error(
        "Blue-Slider Error: The input of 'current_fr_index' must be bigger than zero and less than or equal to the number of slides to be shown!",
      );
      return false;
    }
    ...
    slideTrackContainer.style.width =
      frWidth * frItemsWidth +
      pxItemsWidth +
      frWidth * (wholeItemsCount - frItemsCount - pxItemsCount) +
      slide_gap * wholeItemsCount +
      "px";
    slideTrackContainer.style.transition =
      "transform " + settings.speed + "ms " + settings.ease_function;
    sliderParent.children().each(function () {
      ...
      slideItemContainer.style.transition =
        "all " + settings.speed + "ms " + settings.ease_function;
    ...
    if (
      2 - settings.start_slide_index > current_fr_index ||
      (!settings.current_fr_index_flow
        ? settings.start_slide_index > wholeItemsCount - current_fr_index + 1
        : settings.start_slide_index > wholeItemsCount)
    ) {
      console.error(
        "Blue-Slider Error: The input of 'start_slide_index' is not in correct range!",
      );
      return false;
    }
    ...
    if (start_slide_index >= 0) {
      currentTransX =
        -(frWidth + slide_gap) * start_slide_index + settings.left_padding;
    } else {
      for (var i = 0; i < -start_slide_index; i++) {
        if (activeItemsArray[i].indexOf("fr") > -1) {
          currentTransX +=
            frWidth * parseFloat(activeItemsArray[i]) +
            slide_gap +
            settings.left_padding;
        } else {
          currentTransX +=
            parseFloat(activeItemsArray[i]) + slide_gap + settings.left_padding;
        }
      }
    }
    ...
    slideTracker.on("mouseup", ".slide-item", function (e) {
      if (xClick == e.pageX) {
        ...
        var moveSteps =
          $(this).index() -
          slideTracker.children("." + current_fr_class).index();
        if (moveSteps > 0) {
    ...
    sliderParent.mousedown(function (e) {
      ...
      var newStyles =
        slideTrackerStyles.substring(
          0,
          slideTrackerStyles.indexOf("transition") - 1,
        ) +
        slideTrackerStyles.substring(
          slideTrackerStyles.indexOf(
            ";",
            slideTrackerStyles.indexOf("transition"),
          ) + 1,
        );
    ...
    sliderParent.mousemove(function (e) {
      if (dragEnable) {
        ...
        if (
          (slideTracker.children("." + current_fr_class).index() == 0 &&
            distanceMove > 0) ||
          (slideTracker.children("." + current_fr_class).index() ==
            wholeItemsCount - 1 &&
            distanceMove < 0)
        ...
        slideTracker.css({
          transform:
            "translate3d(" + (distanceMove + currentTransX) + "px, 0, 0)",
        });
      }
    });
    ...
    function slideBackward() {
      if (
        settings.current_fr_index_flow &&
        (current_fr_index_temp > current_fr_index || current_fr_index_temp > slideTracker.children("." + current_fr_class).index()) &&
        current_fr_index_temp > 1
      ) {
        current_fr_index_temp--;
      } else {
        arrowClickEnable = true;
        if (slideTracker.children("." + current_fr_class).index() != 0) {
          start_slide_index--;
          if (
            currentTransX + (frWidth + slide_gap) > settings.left_padding &&
            currentTransX + (frWidth + slide_gap) <=
              sliderParentWidth - settings.right_padding
          ) {
            var itemWidth = frWidth;
            if (
              activeItemsArray[Math.abs(start_slide_index) - 1].indexOf("fr") >
              -1
            ) {
              itemWidth = frWidth * parseFloat(activeItemsArray[Math.abs(start_slide_index) - 1]);
            } else {
              itemWidth = parseFloat(
                activeItemsArray[Math.abs(start_slide_index) - 1],
              );
            }
      ...
      if (
        ...
        (current_fr_index_temp < current_fr_index ||
          wholeItemsCount -
            slideTracker.children("." + current_fr_class).index() -
            1 <=
            activeItemsCount - current_fr_index) &&
        ...
        if (
          ...
          start_slide_index++;
          if (
            currentTransX - (frWidth + slide_gap) >= 0 &&
            currentTransX - (frWidth + slide_gap) <= sliderParentWidth
          ) {
            var itemWidth = frWidth;
            if (
              activeItemsArray[Math.abs(start_slide_index)].indexOf("fr") > -1
            ) {
              itemWidth =
                frWidth *
                parseFloat(activeItemsArray[Math.abs(start_slide_index)]);
            }
            currentTransX -= itemWidth + slide_gap;
          } else {
            currentTransX -= frWidth + slide_gap;
